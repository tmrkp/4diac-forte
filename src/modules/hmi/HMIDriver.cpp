...
std::atomic<bool> HMIDriver::running{true};
...
void HMIDriver::runLater(std::function<void()> task) {
  CCriticalRegion criticalRegion(taskQueueMutex);
  taskQueue.push(std::move(task));
}

lv_display_t *HMIDriver::initHAL(int32_t w, int32_t h) {
